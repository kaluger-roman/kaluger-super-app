...
### Added
- Звуковые сигналы звонка: гудок при дозвоне (у звонящего) и рингтон при входящем вызове (у вызываемого) (f477da0)

### Changed
- При попытке начать демонстрацию экрана, когда собеседник уже её ведёт, показывается подсказка «Собеседник уже демонстрирует экран» вместо неактивной кнопки; вызываемому показывается уведомление «Вызов отменён», если звонящий отменил вызов (83e9163)

### Fixed
- Корректная обработка одновременного включения камеры обеими сторонами во время звонка (perfect negotiation) — видео больше не теряется при гонке переговоров (83e9163)

## 2026-06-04

### Added
- Видеозвонки между репетитором и учеником (WebRTC, peer-to-peer). Репетитор звонит привязанному ученику из карточки ученика, ученик — репетитору из кабинета; вторая сторона видит входящий вызов с именем звонящего и принимает/отклоняет. Медиа идёт напрямую между устройствами, сервер только ретранслирует сигналинг (offer/answer/ICE + жизненный цикл звонка) по существующему WebSocket для обоих пулов (репетиторы `/ws`, ученики `/ws/student`)
- Управление во время звонка: переключение микрофона и камеры с индикаторами у собеседника, режим «только аудио» при отсутствии/запрете камеры (с возможностью включить камеру позже), демонстрация экрана с правилом «один за раз» и аккуратным возвратом к камере по остановке
- Обработка всех краевых состояний с русскими сообщениями: «Ученик сейчас не в сети», «Абонент занят», «Нет ответа» (таймаут 40с), «Вызов отклонён», обрыв соединения и завершение с двух сторон, освобождение камеры/микрофона на любом терминальном переходе
- История звонков: репетитор и ученик видят только свою пару звонков в обратном хронологическом порядке (собеседник, направление, время, длительность, статус) — `GET /api/calls/history` и `GET /api/student/calls/history`, строго ограниченные авторизованным участником
- ICE-конфигурация STUN-first / TURN-fallback: бэкенд чеканит временные HMAC-креды coturn на каждый звонок (секрет не уходит в браузер), STUN-only при незаданном TURN. Новые env: `STUN_URL`, `TURN_URL`, `TURN_SECRET`, `TURN_CREDENTIAL_TTL`
- Новая таблица `call_records` (Prisma-модель + миграция `030_call_records`, enum'ы `CallStatus`/`CallerKind`) — одна запись на каждый завершённый/несостоявшийся звонок пары
