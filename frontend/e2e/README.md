...
**Никогда:** CSS-селекторы по классам (`.MuiButton-root`), nth-child, XPath. Это не e2e-тест, а ловушка для рефакторинга. Линтер (`playwright/no-raw-locators`) не пропускает любой `locator("…")` со строковым селектором.
