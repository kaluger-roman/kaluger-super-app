...
test.describe("Пробный урок без создания ученика", { tag: ["@regression", "@lessons"] }, () => {
  test("учитель создаёт пробный урок без ученика и видит его с пометкой, список учеников не меняется", async ({
    ...
    await expect(dialog.getByLabel("Регулярное занятие (еженедельно)")).toBeHidden();
    ...
    const trialLesson = lessons.find((l) => l.prospectName === "Пётр Пробников");
