...
test.describe("Ученик смотрит свои настройки", { tag: ["@regression", "@lessons"] }, () => {
  const setup = async () => {
  ...
  test("ученик видит имя, подтверждённый email и имя учителя", async ({ page }) => {
  ...
  test("если карточка ученика удалена у учителя — показывается плейсхолдер", async ({ page }) => {
    ...
    await expect(page.getByText("Связь с преподавателем прекращена")).toBeVisible();
  });
});
