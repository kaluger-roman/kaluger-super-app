...
test.describe("Повторная отправка кода верификации", { tag: ["@regression", "@auth"] }, () => {
  test("учитель нажимает повторную отправку кода, получает новый код, вводит его и становится верифицированным", async ({
    ...
    const firstMail = await waitForMail(credentials.email, (m) => Boolean(m.verificationCode));
    ...
    const secondMail = await waitForMail(credentials.email, (m) => Boolean(m.verificationCode));
