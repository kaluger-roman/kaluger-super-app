...
import { registerStudentDirect, seedStudentAuthInBrowser } from "../helpers/student";

test.describe("Подтверждение email ученика", { tag: ["@regression", "@auth"] }, () => {
  ...
  test("ученик вводит 6-значный код и попадает в кабинет", async ({ page }) => {
  ...
  test("кнопка «Отправить заново» уходит в обратный отсчёт после клика", async ({ page }) => {
    ...
    await expect(page.getByRole("button", { name: /Отправить заново через \d+с/ })).toBeDisabled();
  });
});
