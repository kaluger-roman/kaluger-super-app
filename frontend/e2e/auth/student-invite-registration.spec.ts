...
import { createStudentFor, extractInviteToken, issueStudentInvitation } from "../helpers/db";

test.describe("Регистрация ученика по приглашению", { tag: ["@regression", "@auth"] }, () => {
  test("ученик открывает инвайт-ссылку, регистрируется и попадает в кабинет с экраном верификации", async ({
    ...
    const { inviteUrl } = await issueStudentInvitation(tutor.token, student.id);
