...
test.describe("Ежемесячный финансовый отчёт", { tag: ["@regression", "@reports"] }, () => {
  test("учитель видит корректный заработок и задолженность за текущий месяц", async ({
    ...
    for (let i = 0; i < 3; i += 1) {
      const start = new Date(now.getFullYear(), now.getMonth(), 2 + i, 10, 0, 0);
      await createLesson({
        tutorId: userId,
    ...
    const unpaidStart = new Date(now.getFullYear(), now.getMonth(), 5, 14, 0, 0);
    await createLesson({
      tutorId: userId,
      studentId: student.id,
