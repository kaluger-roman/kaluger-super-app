...
test.describe("Включение и выключение учёта налога", { tag: ["@regression", "@profile"] }, () => {
  test("учитель включает учёт налога при наличии периодов", async ({ page, tutor }) => {
    await setTaxPeriodsFor(tutor.userId, [{ startDate: "2024-01-01", rate: 4 }], false);
  ...
  test("учитель не может включить учёт налога без настроенных периодов", async ({
    ...
    await expect(page.getByText(/добавьте хотя бы один период/i)).toBeVisible();
    await expect(taxSwitch).not.toBeChecked();
  });
});
