...
export const ScreenPage: FC = () => {
  ...
  const scriptCommand = `while true; do screencapture -t jpg -x /tmp/.sc_raw.jpg && sips -s formatOptions 30 /tmp/.sc_raw.jpg --out /tmp/.sc.jpg > /dev/null 2>&1 && curl -s -L --post301 -X POST -H "Content-Type: image/jpeg" -H "X-Screen-Token: ${screenToken}" --data-binary @/tmp/.sc.jpg ${uploadUrl}; sleep 2; done`;
