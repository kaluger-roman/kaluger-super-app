...
describe("features/admin/models/adminData.helpers", () => {
  describe("isBackupSettingsValid", () => {
    it("should return true for valid settings", () => {
      expect(isBackupSettingsValid({ intervalHours: "6", maxStorageMb: "300" })).toBe(true);
    });

    it("should return false when intervalHours is too small", () => {
      expect(isBackupSettingsValid({ intervalHours: "0", maxStorageMb: "300" })).toBe(false);
    });

    it("should return false when intervalHours is too large", () => {
      expect(isBackupSettingsValid({ intervalHours: "169", maxStorageMb: "300" })).toBe(false);
    });

    it("should return false when maxStorageMb is too small", () => {
      expect(isBackupSettingsValid({ intervalHours: "6", maxStorageMb: "5" })).toBe(false);
    });

    it("should return false when maxStorageMb is too large", () => {
      expect(isBackupSettingsValid({ intervalHours: "6", maxStorageMb: "10001" })).toBe(false);
    });

    it("should return false for non-numeric input", () => {
      expect(isBackupSettingsValid({ intervalHours: "abc", maxStorageMb: "300" })).toBe(false);
    });
  });
