...
- Follow-up from the local code review (`docs/code-reviews/worktree-chore-lint-m7-e2e/iter-1.json`, 8 findings at or above the threshold): the e2e override turns `no-restricted-imports` off, because the FSD pattern `../**/pages/**` also matches `../pages/StudentsPage` and would reject the page objects `e2e-testing.md` prescribes; the `e2e/pages/` PascalCase override excludes `index.ts`, which CLAUDE.md wants in every folder; the tag selector gained a third form so `test.describe.serial.only(...)` is covered too; `playwright/no-element-handle` and `no-eval` joined the set, and both docs now state what the linter really catches (`no-raw-locators` only sees `locator()` with a string literal — `page.click("…")` or a selector in a variable stay review-checked); `/e2e-check` and `/e2e-hunt` no longer prescribe tags the new rules reject (`@critical`, and dropping `@draft` without putting `@regression` in its place); the password masking of `RegisterFormFields` / `StudentInviteForm` and the `inputmode` of the code inputs, which the replaced CSS locators guarded implicitly, are covered by unit tests.
