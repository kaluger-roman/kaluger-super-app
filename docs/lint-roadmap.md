...
Pre-commit (#27): `.githooks/pre-commit` → `lint-staged` runs `eslint --fix` + `prettier --write` on staged `frontend/src` / `backend/src` files.
...
| # | Task | Status |
| --- | --- | --- |
| 27 | Pre-commit hook: `lint-staged` runs `eslint --fix` + `prettier --write` on staged `frontend/src` / `backend/src` files | ✅ PR7 |
...
- **#27 `--no-stash`** — the stash stack is shared by all worktrees and parallel sessions, and lint-staged would otherwise keep a backup entry there for the whole run. Without the stash, a failed run leaves the tasks' fixes in the index instead of reverting them. Partially staged files still work: their unstaged hunks go to a patch in the worktree's git dir and are re-applied after the tasks.
- **#27 scope** — globs mirror `npm run lint` / `format:check`: ESLint on `src/**/*.{ts,tsx}` (frontend) and `src/**/*.ts` (backend), Prettier on the same plus `js,jsx,json,css,md` in frontend. `frontend/e2e/**` waits for Milestone 7.
- **#28 Prettier 3** — frontend moved from 2.8.8 to 3.x and backend got the same version. The config moved from `frontend/.prettierrc` to the repo root, so both packages share one (CI path filters include `.prettierrc`). Most of the churn was code wrapped at 80 columns under a `printWidth: 100` config. The one-off reformat is a separate commit listed in `.git-blame-ignore-revs`.
- Hook and installer are tested in `scripts/__tests__/{pre-commit-hook,install-git-hooks}.test.mjs` (temp git repos, stub `lint-staged`); a new `scripts` CI job runs `node --test scripts/__tests__/`.
