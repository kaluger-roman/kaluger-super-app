...
The pre-commit hook (`.githooks/pre-commit`) runs `eslint --fix` and `prettier --write` on staged `frontend/src` / `backend/src` files and re-stages the result; CI also runs `npm run format:check`. The hook sees only staged files, so the checks above still apply. Its "Skipping backup because `--no-stash` was used" warning is expected: the stash stack is shared by all worktrees, so lint-staged stays off it. It needs `frontend/node_modules` / `backend/node_modules` — in a fresh worktree, symlink them from the main checkout. If it fails, fix the cause; do not commit with `--no-verify` unless the user asks.
