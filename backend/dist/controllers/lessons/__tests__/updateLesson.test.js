...
const wsManager = __importStar(require("../../../lib/wsManager"));
const recurringHelpers = __importStar(require("../../../services/recurringHelpers"));
describe("updateLesson controller", () => {
    ...
    beforeAll(async () => {
        const user = await prisma_1.default.user.create({
            data: { email: faker_1.faker.internet.email(), password: "x", name: "u" },
        });
        userId = user.id;
    ...
    it("returns 409 on scheduling conflict", async () => {
        ...
        const toUpdate = await prisma_1.default.lesson.create({
            data: {
                tutorId: userId,
                studentId,
                subject: "PHYSICS",
    ...
    it("shifts recurring lessons and handles conflicts", async () => {
        // prepare mocks
        const shiftSpy = jest.spyOn(recurringHelpers, "shiftFutureRecurringLessons");
        shiftSpy.mockImplementation(async () => ({
            shifted: 0,
        ...
        await (0, supertest_1.default)(index_1.app)
            ...
            endTime: new Date(Date.now() + 2 * 24 * 3600 * 1000 + 3600000).toISOString(),
        })
            .expect(500)
            .then((res) => expect(res.body.error).toBeDefined());
    ...
    it("updates price for future recurring lessons and broadcasts websocket on status change", async () => {
        ...
        updatePriceSpy.mockResolvedValue({ updated: 1 });
        // mock ws manager
        const broadcast = jest.fn();
        const wsSpy = jest
            .spyOn(wsManager, "getWebSocketManager")
            .mockReturnValue({ broadcastLessonStatusUpdate: broadcast });
        const base = await prisma_1.default.lesson.create({
            data: {
                tutorId: userId,
                studentId,
    ...
    it("does not call shift or price updater when status is RESCHEDULED", async () => {
        ...
        const r = await prisma_1.default.lesson.create({
            data: {
                tutorId: userId,
                studentId,
                subject: "MATHEMATICS",
    ...
    it("calls shiftFutureRecurringLessons on successful shift and proceeds", async () => {
        const shiftSpy = jest.spyOn(recurringHelpers, "shiftFutureRecurringLessons");
        shiftSpy.mockResolvedValue({ shifted: 2 });
        const r = await prisma_1.default.lesson.create({
            data: {
                tutorId: userId,
                studentId,
...
//# sourceMappingURL=updateLesson.test.js.map