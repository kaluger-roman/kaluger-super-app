...
describe("getLessons controller", () => {
    ...
    it("returns single lesson by id and 404 for others", async () => {
        ...
        const other = await prisma_1.default.user.create({
            data: { email: faker_1.faker.internet.email(), password: "x", name: "o" },
        });
        const otherToken = (0, auth_1.generateToken)({ userId: other.id, email: other.email });
        await (0, supertest_1.default)(index_1.app)
            .get(`/api/lessons/${lesson.id}`)
            .set("Authorization", `Bearer ${otherToken}`)
            .expect(404);
        await prisma_1.default.user.delete({ where: { id: other.id } });
    });
    ...
    it("supports upcoming and status filters", async () => {
        ...
        const inProgress = await prisma_1.default.lesson.create({
            data: {
                tutorId: userId,
                studentId,
                subject: "PHYSICS",
        ...
        const scheduledFuture = await prisma_1.default.lesson.create({
            data: {
                tutorId: userId,
                studentId,
                subject: "PHYSICS",
        ...
        await (0, supertest_1.default)(index_1.app)
            ...
            .expect(200)
            .then((res) => {
            const ids = res.body.lessons.map((l) => l.id);
        ...
        await (0, supertest_1.default)(index_1.app)
            ...
            .expect(200)
            .then((res) => {
            const statuses = res.body.lessons.map((l) => l.status);
            expect(statuses.every((s) => s === "COMPLETED")).toBe(true);
        });
    });
    it("filters by startDate and endDate", async () => {
        const localStudent = await prisma_1.default.student.create({
            data: {
                name: faker_1.faker.person.fullName(),
                contactMethod: "WHATSAPP",
                tutorId: userId,
        ...
        d3.setSeconds(0, 0);
        const L1 = await prisma_1.default.lesson.create({
            data: {
                tutorId: userId,
                studentId: sid,
                subject: "MATHEMATICS",
        ...
        const L2 = await prisma_1.default.lesson.create({
            data: {
                tutorId: userId,
                studentId: sid,
                subject: "MATHEMATICS",
        ...
        const L3 = await prisma_1.default.lesson.create({
            data: {
                tutorId: userId,
                studentId: sid,
                subject: "MATHEMATICS",
        ...
        await prisma_1.default.lesson.deleteMany({ where: { studentId: sid } });
        await prisma_1.default.student.delete({ where: { id: sid } });
    });
    it("supports multi-status filter (comma separated)", async () => {
        const a = await prisma_1.default.lesson.create({
            data: {
                tutorId: userId,
                studentId,
                subject: "PHYSICS",
        ...
        const b = await prisma_1.default.lesson.create({
            data: {
                tutorId: userId,
                studentId,
                subject: "PHYSICS",
    ...
    it("onlyUnpaid excludes lessons with price 0", async () => {
        const paidZero = await prisma_1.default.lesson.create({
            data: {
                tutorId: userId,
                studentId,
                subject: "MATHEMATICS",
        ...
        const paidNonZero = await prisma_1.default.lesson.create({
            data: {
                tutorId: userId,
                studentId,
                subject: "MATHEMATICS",
    ...
    it("orders ascending when upcoming=true", async () => {
        const now = new Date();
        const t1 = await prisma_1.default.lesson.create({
            data: {
                tutorId: userId,
                studentId,
                subject: "PHYSICS",
        ...
        const t2 = await prisma_1.default.lesson.create({
            data: {
                tutorId: userId,
                studentId,
                subject: "PHYSICS",
        ...
        await (0, supertest_1.default)(index_1.app)
            ...
            .expect(200)
            .then((res) => {
            const starts = res.body.lessons.map((l) => new Date(l.startTime).getTime());
            // ensure ascending
    ...
    it("weekly ignores upcoming query param", async () => {
        ...
        const old = await prisma_1.default.lesson.create({
            data: {
                tutorId: userId,
                studentId,
                subject: "PHYSICS",
    ...
    it("non-weekly non-upcoming returns descending order by default", async () => {
        ...
        const c = await prisma_1.default.lesson.create({
            data: {
                tutorId: userId,
                studentId,
                subject: "MATHEMATICS",
    ...
    it("upcoming=true without currentTime is ignored and behaves as status filter absent", async () => {
        ...
        const now = new Date();
        const fut = await prisma_1.default.lesson.create({
            data: {
                tutorId: userId,
                studentId,
                subject: "MATHEMATICS",
    ...
    it("onlyUnpaid true excludes lessons where isPaid = true", async () => {
        const paid = await prisma_1.default.lesson.create({
            data: {
                tutorId: userId,
                studentId,
                subject: "MATHEMATICS",
        ...
        const unpaid = await prisma_1.default.lesson.create({
            data: {
                tutorId: userId,
                studentId,
                subject: "MATHEMATICS",
...
//# sourceMappingURL=getLessons.test.js.map